...
import { clearErrors, createProductNew } from "../../actions/ProductActions";
import { Button } from "@material-ui/core";
import { NEW_PRODUCT_RESET } from "../../constans/ProductConstans";
import { ToastContainer, toast } from "react-toastify";
import "./newProduct.css";

const CreateProductNew = ({ history }) => {
  const dispatch = useDispatch();
  const { loading, error, success } = useSelector(
    (state) => state.createProductNew
  );
  ...
  const createProductSubmitHandler = (e) => {
    ...
    dispatch(createProductNew(myForm));
  };
