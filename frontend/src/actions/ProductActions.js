...
export const getProductNew =
  ...
  async (dispatch) => {
    try {
      ...
      let link = `/api/v2/products/new/new?keyword=${keyword}&page=${currentPage}`;

      if (category) {
        link = `/api/v2/products/new/new?keyword=${keyword}&page=${currentPage}&category=${category}`;
      }
      const { data } = await axios.get(link);
...
export const getProductOffer =
  ...
  async (dispatch) => {
    try {
      ...
      let link = `/api/v2/products/offer?keyword=${keyword}&page=${currentPage}`;

      if (category) {
        link = `/api/v2/products/offer?keyword=${keyword}&page=${currentPage}&category=${category}`;
      }
      const { data } = await axios.get(link);
...
export const createProductOffer = (productData) => async (dispatch) => {
  try {
    dispatch({ type: NEW_PRODUCT_REQUEST });
